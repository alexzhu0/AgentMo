...
export function buildHandoffPackage(blueprint, options = {}) {
  ...
  const handoff = {
    ...
    requiredInputs: ["validated blueprint", "discovery pack", "user-need report", "build-state", "run-state", "run-eval"],
    requiredOutputs: ["runtime scaffold", "bounded evidence", "birth report", "verification log"],
...
function renderVerify(blueprint, handoff) {
  ...
  lines.push("", "## Handoff boundary", "- Handoff does not certify runtime behavior.", "- Handoff does not certify domain quality.", "- Birth-report must fail closed on missing build/run/eval evidence.");
...
function renderTestPlan(blueprint, handoff) {
  const lines = [`# Test plan for ${blueprint.agent_id}`, "", "## Required", "- `agentmo validate <blueprint>`", `- \`agentmo scaffold <blueprint> --target ${handoff.target} --out <runtime-output>\``, "- `agentmo birth-report <blueprint> --build-state <state> --run-state <state> --run-eval <eval> --expect-status declared`", "- `npm run check`", "", "## Promotion only", "- Isolated live run.", "- `run-eval --expect-status success`.", "- `birth-report --expect-status success`."];
  return `${lines.join("\n")}\n`;
}
...
function renderEvidenceRequirements(blueprint, handoff) {
  const lines = [`# Evidence requirements for ${blueprint.agent_id}`, "", "- No credential values.", "- No raw transcripts.", "- No raw tool bodies.", "- No production OpenClaw state for declared/live-success MVP birth evidence.", "- Store bounded summaries, artifact paths, hashes/statuses, and explicit missing-evidence notes.", "", "## Required artifacts"];
