...
import { buildRuntimePlan, FRESH_RUN_SESSION_KEY_PLACEHOLDER, RUNTIME_PLAN_SCHEMA_VERSION } from "../src/runtime-plan.js";
...
describe("runtime plan", () => {
  it("builds deterministic OpenClaw runtime plans without writing files", async () => {
    ...
    assert.deepEqual(first.command.args, ["agent", "--agent", "win9", "--session-key", FRESH_RUN_SESSION_KEY_PLACEHOLDER, "--message", "ping"]);
  ...
  it("builds source checkout command plans", async () => {
    ...
    assert.deepEqual(plan.command.args, ["openclaw", "agent", "--agent", "win9-main", "--session-key", "smoke-session", "--message", "ping"]);
  });
