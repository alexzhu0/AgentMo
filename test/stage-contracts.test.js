...
describe("stage contract independence", () => {
  ...
  it("Stage 2 drafts a valid blueprint from a prebuilt discovery-db fixture plus user need", async () => {
    ...
    assert.deepEqual(await listRelativeFiles(root), ["support-triage.agentmo.json"]);
  });
  ...
  it("Stage 3 handoff admits an existing design contract without discovery or draft ancestry", async () => {
    ...
    const validate = await runCli(["validate", SUPPORT_BLUEPRINT]);
    assert.equal(validate.code, 0, validate.stderr);

    const handoffOut = path.join(root, "handoff");
    const handoff = await runCli(["handoff", SUPPORT_BLUEPRINT, "--target", "openclaw", "--out", handoffOut, "--json"]);
    ...
    assert.equal(handoffJson.handoff.provenanceReferences.includes("review reference: blueprint-draft:agentmo.discovery-db.v1+agentmo.user-need.v1"), true);
    assert.equal(
      handoffJson.handoff.provenanceReferences.includes("discovery pack (AgentMo-generated design review reference)"),
      true,
    );
    assert.equal(
      handoffJson.handoff.provenanceReferences.includes("user-need report (AgentMo-generated design review reference)"),
      true,
    );
  ...
  it("Stage 3 declared evidence remains non-certifying without live or domain eval evidence", async () => {
    ...
    const scaffold = await runCli(["scaffold", SUPPORT_BLUEPRINT, "--target", "openclaw", "--out", scaffoldOut]);
    ...
    const run = await runCli([
      ...
      "--message",
      "Classify a refund ticket with missing order context.",
      "--out",
      runOut,
      "--json",
    ]);
