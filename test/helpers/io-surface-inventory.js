...
exactModuleSurfaceGroup("src/builder-append-only-authority.js", "phase-02-plan-19", "gated", `
  ...
  exactModuleSurfaceGroup("src/builder-behavior-eval.js", "phase-02-plan-15", "gated", `
    ...
    1011:stream-write:stdin.end
    1130:stream-write:stdin.end
  `),
...
// Bootstrap release delivery is pipe-fed from retained, digest-verified bytes.
// It creates no snapshot paths; the bridge reads its authenticated descriptor.
reconcileExactModuleSurfaces(IO_SURFACE_ALLOWLIST, [
  exactModuleSurfaceGroup("plugin/hooks/agentmo-hook.js", "phase-02-plan-17", "gated", `
    197:filesystem-read:fs.lstat
    199:filesystem-read:fs.realpath
    212:filesystem-read:fs.realpath
    214:filesystem-read:fs.lstat
    215:filesystem-read:fs.lstat
    216:filesystem-read:fs.realpath
    312:filesystem-read:fs.lstat
    313:filesystem-read:fs.realpath
    324:filesystem-read:fs.lstat
    325:filesystem-read:fs.realpath
    329:filesystem-open:fs.open
    330:file-handle-read:FileHandle.stat
    333:file-handle-read:FileHandle.stat
    334:filesystem-read:fs.lstat
    405:filesystem-read:fs.lstat
    407:filesystem-read:fs.realpath
    410:filesystem-open:fs.open
    411:file-handle-read:FileHandle.stat
    414:filesystem-read:fs.lstat
    423:file-handle-read:FileHandle.stat
    425:file-handle-read:FileHandle.readFile
    646:filesystem-read:fs.lstat
    648:filesystem-open:fs.open
    652:file-handle-read:FileHandle.stat
    653:filesystem-read:fs.lstat
    679:filesystem-read:fs.lstat
    695:file-handle-read:FileHandle.stat
    696:filesystem-read:fs.lstat
    704:filesystem-read:fs.lstat
    768:filesystem-read:fs.readdir
    792:filesystem-read:fs.lstat
    804:filesystem-read:fs.lstat
    805:filesystem-read:fs.readlink
    806:filesystem-read:fs.lstat
    1806:process-spawn:child_process.spawn
    1955:stream-write:stdin.end
    1956:stream-write:null.end
    1957:stream-write:null.end
    2059:process-output:process.stdout.write
  `),
  exactModuleSurfaceGroup("src/builder-bootstrap-snapshot.js", "phase-02-plan-17", "gated", `
    118:filesystem-read:fs.fstatSync
    122:filesystem-read:fs.createReadStream
  `),
...
reconcileExactModuleSurfaces(IO_SURFACE_ALLOWLIST, [
  ...
  exactModuleSurfaceGroup("src/openclaw-install-transaction.js", "phase-04-plan-15", "gated", `
    ...
    681:durable-loader:loadAdmittedArtifact
    797:filesystem-open:fs.open
    801:file-handle-read:FileHandle.stat
    805:file-handle-read:FileHandle.readFile
    806:file-handle-read:FileHandle.stat
    807:filesystem-read:fs.lstat
    1797:filesystem-read:fs.realpath
    1798:filesystem-read:fs.lstat
    1977:filesystem-read:fs.access
    1987:filesystem-read:fs.access
  `),
  exactModuleSurfaceGroup("src/openclaw-install-transaction.js", "phase-04-plan-16", "gated", `
    963:durable-loader:loadAdmittedArtifact
    974:durable-loader:loadAdmittedArtifact
  `),
  ...
  exactModuleSurfaceGroup("src/openclaw-process-supervisor.js", "phase-04-plan-19", "gated", `
    47:filesystem-lifecycle:fs.mkdtemp
    50:filesystem-lifecycle:fs.chmod
    180:filesystem-open:fs.open
    187:file-handle:FileHandle.writeFile
    188:file-handle-lifecycle:FileHandle.sync
    386:filesystem-open:fs.open
    390:file-handle-read:FileHandle.stat
    391:file-handle-read:FileHandle.readFile
    392:file-handle-read:FileHandle.stat
    393:filesystem-read:fs.lstat
    413:filesystem-open:fs.open
    419:file-handle:FileHandle.writeFile
    420:file-handle-lifecycle:FileHandle.sync
    430:filesystem-open:fs.open
    435:filesystem-read:fs.lstat
    450:filesystem-open:fs.open
    456:file-handle-read:FileHandle.stat
    469:managed-filesystem:handle.chmod
    470:file-handle-lifecycle:FileHandle.sync
    480:file-handle-read:FileHandle.stat
    486:file-handle-read:FileHandle.stat
    503:file-handle-read:FileHandle.read
    511:filesystem-read:fs.realpath
    535:filesystem-read:fs.lstat
    547:filesystem-open:fs.open
    551:file-handle-read:FileHandle.stat
    552:file-handle-read:FileHandle.readFile
    553:file-handle-read:FileHandle.stat
    554:filesystem-read:fs.lstat
  `),
  exactModuleSurfaceGroup("src/openclaw-safe-fs.js", "phase-04-plan-13", "gated", `
    172:filesystem-lifecycle:fs.mkdtemp
    175:filesystem-lifecycle:fs.chmod
    266:filesystem-open:fs.open
    277:file-handle-read:FileHandle.stat
    278:filesystem-read:fs.lstat
    332:file-handle:FileHandle.writeFile
    334:file-handle-lifecycle:FileHandle.sync
    525:filesystem-read:fs.lstat
    532:filesystem-lifecycle:fs.mkdtemp
    535:filesystem-lifecycle:fs.chmod
    537:filesystem-open:fs.open
    544:file-handle:FileHandle.writeFile
    545:file-handle-lifecycle:FileHandle.sync
    760:managed-writer:stdin.write
    816:stream-write:stdin.end
    824:filesystem-read:fs.realpath
    858:filesystem-lifecycle:fs.mkdtemp
    861:filesystem-lifecycle:fs.chmod
    1039:filesystem-open:fs.open
    1043:file-handle-read:FileHandle.stat
    1052:file-handle-read:FileHandle.readFile
    1053:file-handle-read:FileHandle.stat
    1054:filesystem-read:fs.lstat
    1073:filesystem-open:fs.open
    1078:file-handle-read:FileHandle.stat
    1079:filesystem-read:fs.lstat
    1197:filesystem-read:fs.lstat
    1331:filesystem-open:fs.open
    1337:file-handle-lifecycle:FileHandle.sync
    1347:filesystem-open:fs.open
    1354:file-handle:FileHandle.writeFile
    1355:file-handle-lifecycle:FileHandle.sync
    1356:file-handle-read:FileHandle.stat
  `),
  exactModuleSurfaceGroup("src/openclaw-safe-fs.js", "phase-04-plan-19", "gated", `
    944:filesystem-open:fs.open
    949:filesystem-read:fs.lstat
    966:filesystem-open:fs.open
    972:file-handle-read:FileHandle.stat
    987:managed-filesystem:handle.chmod
    988:file-handle-lifecycle:FileHandle.sync
    1000:file-handle-read:FileHandle.stat
    1008:file-handle-read:FileHandle.stat
    1027:file-handle-read:FileHandle.read
  `),
  exactModuleSurfaceGroup("src/openclaw-probe.js", "phase-04-plan-12", "gated", `
    95:durable-loader:loadAdmittedArtifact
    101:durable-loader:loadAdmittedArtifact
    123:filesystem-lifecycle:fs.mkdtemp
    124:filesystem-lifecycle:fs.chmod
    137:filesystem-lifecycle:fs.mkdir
    138:filesystem-lifecycle:fs.chmod
    141:filesystem-read:fs.lstat
    401:filesystem-read:fs.lstat
    407:filesystem-read:fs.realpath
    416:filesystem-open:fs.open
    420:file-handle-read:FileHandle.stat
    429:file-handle-read:FileHandle.stat
    471:file-handle-read:FileHandle.read
    484:filesystem-open:fs.open
    492:file-handle:FileHandle.writeFile
    493:file-handle-lifecycle:FileHandle.sync
    494:managed-filesystem:writableHandle.chmod
    495:file-handle-read:FileHandle.stat
    496:filesystem-open:fs.open
    500:file-handle-read:FileHandle.stat
    506:filesystem-lifecycle:fs.unlink
    507:file-handle-read:FileHandle.stat
    533:filesystem-open:fs.open
    537:filesystem-open:fs.open
    538:file-handle-read:FileHandle.stat
    539:file-handle-read:FileHandle.stat
    540:filesystem-read:fs.lstat
    567:filesystem-read:fs.lstat
    574:file-handle-read:FileHandle.stat
    575:filesystem-read:fs.lstat
    586:filesystem-read:fs.lstat
    595:filesystem-open:fs.open
    597:file-handle-read:FileHandle.stat
    598:file-handle-read:FileHandle.stat
    604:file-handle-read:FileHandle.stat
  `),
