...
decision-ledger canonicalize-entry --entry <unapproved-entry> --out <absent-canonical-entry>
decision-ledger append --journal <ledger> --entry <canonical-entry> --digest decision-entry=<exact-canonical-bytes> [--expected-head-digest <exact-head>]
...
For an unapproved Decision Entry draft, run `decision-ledger canonicalize-entry` before calculating or displaying its digest. It creates a distinct absent artifact with `sourceRefs`, `decisionRefs`, and `requirementRefs` in strictly ascending UTF-8 byte order; duplicates are rejected rather than removed. The writer fixes that order before serialization, verifies the exact one-link private-stage bytes, and reports the digest of those bytes only after create-only publication to an absent name. `append` and artifact admission only verify those exact bytes and never sort, rewrite, or otherwise alter an already-digest-bound artifact. Canonicalization is supported only on AgentMo's Darwin/Linux POSIX no-follow platforms and requires a caller-owned, non-group/world-writable output parent plus exclusive same-UID control of the output parent, private stage, and target namespace while the command runs. It is not a same-UID concurrent filesystem transaction: it does not claim to defend against concurrent link, rename, unlink, or write mutation. Observable pre-publication identity or symlink anomalies fail closed, and the command never rolls back or deletes a published output by pathname. The Plan ledger's only stale-head option is `--expected-head-digest`; `--expected-head-sha256` is Builder-only and is rejected by `decision-ledger`.
