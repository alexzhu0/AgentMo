...
- 当前重点：Phase 01.2 的 Node 20 core receipt、OpenClaw pre-effect gate、launcher/shell/trust-anchor closure 与 compatibility matrix 已完成有界复跑；本记录与实现必须通过同一 `main` checkpoint 同步，GitHub Release 仍是独立的后续发布动作。每个日期型 release 记录都应作为对应 GitHub Release 的正文同步，而不是作为附件 asset。
